...
int main(){
    ...
    Shader cubeShader("../cubemap.vs.glsl", "../cubemap.fs.glsl");
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (float)WIDTG / (float)HEIGHT, 0.1f, 100.0f);
        ...
        model = glm::translate(model, glm::vec3(-1.0f, EPS, -1.0f));
        myShader.setMat4("model", model);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(2.0f, EPS, 0.0f));
