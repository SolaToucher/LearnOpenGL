...
vec3 CalcSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir){
    ...
    float intensity = 0;
    if(light.cutOff != light.outerCutOff)intensity = clamp((theta-light.outerCutOff) / (light.cutOff-light.outerCutOff), 0.0f, 1.0f);
    else if(theta > light.cutOff) intensity = 1.0f;
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir){
    ...
    vec3 specular = spec * light.specular * texture(material.diffuse, TexCoords).rgb;
